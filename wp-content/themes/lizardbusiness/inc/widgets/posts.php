...
class lzPosts extends WP_Widget 
{
    function __construct() 
    {
		...
		$this->WP_Widget('lzPosts', '&raquo; Posts with Images', $widget_options, $control_options);
    }
        	...
        	<?php
				...
				foreach($posts as $p) {
                    ...
                    <div class='lz-sidebar-post'>
						...
						<?php if ( $instance['display_date']||$instance['display_title'] ) { ?>
						...
						<?php if ( $instance['display_title'] ) { ?><div class='title'><a href="<?php echo get_permalink($p->ID); ?>" rel="bookmark" title="<?php echo $p->post_title; ?>"><?php echo $p->post_title?></a></div><?php } ?>
						<div class='clear'></div>
						</div>
						<?php } ?>
						
						
						<?php if ($instance['display_featured_image'] && has_post_thumbnail($p->ID) ) {
							echo get_the_post_thumbnail($p->ID,array(56,56), array());
						} ?>
						
                        
						<?php echo '<p>'.mb_substr(strip_tags( apply_filters('the_content', $p->post_content) ),0,$instance['excerpt_length']).'</p>'; ?>
                    </div>
                <?php
                }
                endif;
...
add_action('widgets_init', create_function('', 'return register_widget("lzPosts");'));
?>
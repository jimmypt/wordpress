...
class lzComments extends WP_Widget {
    function __construct() {
		...
		$this->WP_Widget('lzComments', '&raquo; Comments with Avatars', $widget_options, $control_options);
    }
                        ...
                        <div class="lz-sidebar-comment">
                            <?php 
                                ...
                                if( $instance['display_avatar']) { ?>
                                    <div class='avatar-container' style='width:<?php echo $instance['avatar_size']?>px'><?php echo get_avatar( $comment, $instance['avatar_size'] )?></div><?php 
                                } 
                            
                                if($instance['display_comment'] || $instance['display_read_more'] || $instance['display_avatar']) {
                                       if($instance['display_comment']) { 
											echo '<p class="comment">'.mb_substr(strip_tags($comment->com_excerpt),0,$instance['comment_length']).'...</p>';
                                        }	
									if($instance['display_author']) { echo '<strong>&mdash;&nbsp;'.$comment->comment_author.' <a href="'.$com_link.'">'.$instance['read_more_text'].'</a></strong>'; }
                                } else {
...
add_action('widgets_init', create_function('', 'return register_widget("lzComments");'));
?>